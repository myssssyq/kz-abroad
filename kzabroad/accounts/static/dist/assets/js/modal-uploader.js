...
if ($('.modal-uploader').length) {
  ...
  modalUploader.on("addedfile", function (file) {
    //Get the file count
    var count = modalUploader.files.length; //Update global count variable

    uploadCount = uploadCount + 1; // Hookup the start button

    file.previewElement.querySelector(".start").onclick = function () {
      modalUploader.enqueueFile(file);
    };

    file.previewElement.id = 'uploaded-file-' + uploadCount; //Set the new file count
